...
@section('title', __('auth_ui.reset_confirm_title_suffix'))

@section('content')
<div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full">
        <div class="bg-white rounded-lg shadow-md p-8">
            <h2 class="text-2xl font-bold text-center text-gray-900 mb-6">{{ __('auth_ui.reset_confirm_heading') }}</h2>
            ...
            <form method="POST" action="{{ route('password.update.' . app()->getLocale()) }}">
                ...
                <div class="mb-4">
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-1">{{ __('auth_ui.reset_new_password') }}</label>
                ...
                <div class="mb-6">
                    <label for="password_confirmation" class="block text-sm font-medium text-gray-700 mb-1">{{ __('auth_ui.reset_confirm_password') }}</label>
                ...
                <button type="submit"
                    ...
                    {{ __('auth_ui.reset_confirm_submit') }}
                </button>
            </form>

            <p class="mt-6 text-center text-sm text-gray-600">
                <a href="{{ route('login') }}" class="text-blue-600 hover:text-blue-800 font-medium">{{ __('auth_ui.back_to_login') }}</a>
            </p>
        </div>
    </div>
</div>
