...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="flex gap-10">
        ...
        <div class="flex-1 min-w-0">
            <h1 class="font-display font-bold text-2xl text-slate-900 mb-6">{{ $isEn ? 'Subscription' : 'Abonnement' }}</h1>

            {{-- Aktueller Plan --}}
            <div class="bg-white border border-slate-200 rounded-xl p-6 mb-6">
                <h2 class="font-display font-bold text-lg text-slate-900 mb-4">{{ $isEn ? 'Current Plan' : 'Aktueller Plan' }}</h2>
                ...
                @if($subscription && $subscription->status === 'active')
                    <div class="flex items-start gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-500 mt-0.5">{{ $isEn ? 'Your subscription is active.' : 'Ihr Abonnement ist aktiv.' }}</p>
                            <p class="text-sm text-slate-700 mt-2 font-medium">
                                {{ $isEn
                                    ? 'Next payment on ' . $subscription->current_period_end->format('m/d/Y') . ': '
                                    : 'Nächste Zahlung am ' . $subscription->current_period_end->format('d.m.Y') . ': ' }}<span class="text-brand-600">&euro;39,99</span>
                            </p>
                        </div>
                    </div>
                ...
                @elseif($subscription && $subscription->status === 'trialing')
                    <div class="flex items-start gap-4 mb-4">
                        ...
                        <div>
                            <h3 class="font-display font-bold text-slate-900">sofortpdf Pro — {{ $isEn ? 'Trial' : 'Testphase' }}</h3>
                            <p class="text-sm text-slate-500 mt-0.5">{{ $isEn ? 'You are trying sofortpdf Pro for free.' : 'Sie testen sofortpdf Pro kostenlos.' }}</p>
                            <p class="text-sm text-slate-700 mt-2 font-medium">
                                {{ $isEn
                                    ? 'Your trial ends on ' . $subscription->trial_ends_at->format('m/d/Y')
                                    : 'Ihr Testzeitraum endet am ' . $subscription->trial_ends_at->format('d.m.Y') }}
                            </p>
                        </div>
                    </div>
                ...
                @elseif($subscription && $subscription->status === 'canceled')
                    <div class="flex items-start gap-4 mb-4">
                        ...
                        <div>
                            <h3 class="font-display font-bold text-slate-900">sofortpdf Pro — {{ $isEn ? 'Canceled' : 'Gekündigt' }}</h3>
                            <p class="text-sm text-slate-500 mt-0.5">{{ $isEn ? 'Your subscription has been canceled.' : 'Ihr Abonnement wurde gekündigt.' }}</p>
                            <p class="text-sm text-slate-700 mt-2 font-medium">
                                {{ $isEn
                                    ? 'Access until ' . $subscription->current_period_end->format('m/d/Y')
                                    : 'Zugang bis ' . $subscription->current_period_end->format('d.m.Y') }}
                            </p>
                        </div>
                    </div>
                ...
                @else
                    <div class="flex items-start gap-4 mb-4">
                        ...
                        <div>
                            <h3 class="font-display font-bold text-slate-900">{{ $isEn ? 'No Subscription' : 'Kein Abonnement' }}</h3>
                            <p class="text-sm text-slate-500 mt-0.5">{{ $isEn ? 'You do not have an active subscription.' : 'Sie haben derzeit kein aktives Abonnement.' }}</p>
                            <a href="{{ route('checkout.start') }}" class="inline-flex items-center gap-2 mt-3 px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white text-sm font-medium rounded-lg shadow-sm shadow-brand-600/25 hover:shadow-brand-600/40 transition-all">
                                {{ $isEn ? 'Subscribe now' : 'Jetzt abonnieren' }}
                            </a>
                        </div>
                    </div>
                @endif
            ...
            @if($subscription && in_array($subscription->status, ['active', 'trialing']))
                <div class="bg-white border border-slate-200 rounded-xl p-6">
                    <h2 class="font-display font-bold text-lg text-slate-900 mb-4">{{ $isEn ? 'Management' : 'Verwaltung' }}</h2>
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <a href="{{ route('dashboard.billing.portal') }}"
                           class="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 hover:border-slate-300 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors">
                            ...
                            {{ $isEn ? 'Change payment method' : 'Zahlungsmethode ändern' }}
                        </a>
                        ...
                        <button onclick="document.getElementById('cancel-modal').classList.remove('hidden')"
                            ...
                            {{ $isEn ? 'Cancel subscription' : 'Abonnement kündigen' }}
                        </button>
                    </div>
                </div>
                ...
                <div id="cancel-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
                        <h3 class="font-display font-bold text-lg text-slate-900 mb-2">{{ $isEn ? 'Cancel subscription?' : 'Abonnement kündigen?' }}</h3>
                        <p class="text-sm text-slate-600 mb-6">
                            {{ $isEn
                                ...
                                : 'Sind Sie sicher, dass Sie Ihr Abonnement kündigen möchten? Sie behalten Ihren Zugang bis zum Ende des aktuellen Abrechnungszeitraums.' }}
                        </p>
                        <div class="flex gap-3 justify-end">
                            ...
                            <form method="POST" action="{{ route('dashboard.billing.cancel') }}">
                                ...
                                <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg shadow-sm transition-colors">
                                    {{ $isEn ? 'Yes, cancel subscription' : 'Ja, Abonnement kündigen' }}
                                </button>
                            </form>
                        </div>
                    </div>
